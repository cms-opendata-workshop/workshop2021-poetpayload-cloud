...
if doPat:
	...
	process.myjets= cms.EDAnalyzer('PatJetAnalyzer',
				       ...
				       jecUncName = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'Uncertainty_AK5PF.txt'), 
				       jerResName = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/JetResolutionInputAK5PF.txt')         
				       )
else:
	if not isData:
	...
	process.myjets= cms.EDAnalyzer('JetAnalyzer',
				       ...
				       isData = cms.bool(isData),
				       jecL1Name = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'L1FastJet_AK5PF.txt'), 
				       jecL2Name = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'L2Relative_AK5PF.txt'),
				       jecL3Name = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'L3Absolute_AK5PF.txt'),
				       jecResName = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'L2L3Residual_AK5PF.txt'),
				       jecUncName = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/'+JecString+'Uncertainty_AK5PF.txt'),
				       jerResName = cms.FileInPath('workshop2021-poetpayload-cloud/PhysObjectExtractor/JEC/JetResolutionInputAK5PF.txt')
				       )
...
if doPat:
	process.p = cms.Path(process.patDefaultSequence+process.myevents+process.myelectrons+process.mymuons+process.myphotons+process.myjets+process.mymets+process.mytaus+process.mytrigEvent+process.mypvertex+process.mygenparticle+process.mytriggers)
else: 
	if isData: process.p = cms.Path(process.myevents+process.myelectrons+process.mymuons+process.myphotons+process.myjets+process.mymets+process.mytaus+process.mytrigEvent+process.mypvertex+process.mygenparticle+process.mytriggers)
	else: process.p = cms.Path(process.selectedHadronsAndPartons * process.jetFlavourInfosAK5PFJets * process.myevents+process.myelectrons+process.mymuons+process.myphotons+process.myjets+process.mymets+process.mytaus+process.mytrigEvent+process.mypvertex+process.mygenparticle+process.mytriggers)
